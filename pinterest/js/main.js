/* 유지 보수를 위해 함수를 이용해 기능별로 분리하는 작업 전
...
// 유지 보수에 적합한 함수 분리 작업
// 자주 수정할 수 있는 정봇값들을 최상단에 전역 변수로 설정, 요소, 요소의 스타일에 설정된 값 등등
const frame = "section";
const box = "article";
const speed = "0.5s";
...
//이미지 소스를 활용한 모든 컨텐츠의 로딩이 완료되면 실행
window.addEventListener("load", ()=>{
	init(); // 화면 초기화 함수 호출
...
//화면 초기화 함수 정의
function init() {
	//변수 grid에 담길 결과값이 다른 함수인 filter에서도 활용되어야 하므라 전역 변수로 선언
